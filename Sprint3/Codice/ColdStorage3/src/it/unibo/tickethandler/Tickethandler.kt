...
class Tickethandler ( name: String, scope: CoroutineScope, isconfined: Boolean=false  ) : ActorBasicFsm( name, scope, confined=isconfined ){
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		val interruptedStateTransitions = mutableListOf<Transition>()
				...
				state("sendrejectedticketnumber") { //this:State
					action { //it:State
						if( checkMsgContent( Term.createTerm("getrejectedtickets(NO_PARAMS)"), Term.createTerm("getrejectedtickets(NO_PARAM)"), 
						                        currentMsg.msgContent()) ) { //set msgArgList
								answer("getrejectedtickets", "rejectedtickets", "rejectedtickets($Rejected)"   )  
						}
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
